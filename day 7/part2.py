...
def findFoldersThatCanBeDeleted(inputRoot: Folder, inputTarget, inputList):
    for fold in inputRoot.folders:
        findFoldersThatCanBeDeleted(fold, inputTarget, inputList)

    if inputRoot.size >= inputTarget:
        inputList.append(inputRoot)


def findMinimumValue(list):
    min = 9999999999
    for item in list:
        if item.size < min:
            min = item.size
...
totalSpace = 70000000
needed = 30000000
target = root.size + needed - totalSpace
list = []
findFoldersThatCanBeDeleted(root, target, list)
print(list)
print(findMinimumValue(list))
